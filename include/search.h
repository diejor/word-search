...
// =----------------- PUBLIC INTERFACE -----------------=
    ...
    // search result unwrapper functions
    ...
    vector<tuple<string, int, int, Direction>> movies_in_soup(const vector<vector<char>>& soup, const vector<string>& movies);

    vector<string> unzip_titles(vector<tuple<string, int, int, Direction>> movies_found);
