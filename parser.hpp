...
/* 
    The parser reads token liens from the input file. A token line is defined as a line of characters that satisfy the is_token_line function. Token liens are then grouped into blocks of tokens. A block of tokens is defined as a successive group of token lines. The parser expects two blocks of tokens from the input file:
        1. The token block of the soup. The first line consists of the dimensions (rows cols), the rows determine how many subsequent lines will be read to form the soup of letters.
        2. The token block of movie titles to search which is of an arbitrary size and should consume the rest of the input file when read.
*/
namespace parser {
    ...
    /*
        This function defines what is a token line which is any line that the first character doesn't contain any NOT_TOKENS char. 
    ...
    */
    ...
    /*
        This function is called to assert that the internal pointer of the ifstream file is not at the end of the file. Note that the end of the file is a token line (see is_token_line function). 
        
        If it is the case that the pointer is at the end of the file and this functions was called, it means the program tried to read a non-empty token line (e.i the rows and columns dimensions line), but the file was empty. The program should recover at this point asking a new input file to the user. Since working this without having advance tools, such as the option<type> feature, its a nightmare in my opinion, I decided to just assume that the input file will always have the expected two blocks of tokens.
    */
    ...
    /*
        ...
        One result of the state of the ifstream file passed, the function always advances it's internal pointer to the next token line.
    */
    ...
    */
    ...
    /*
        It's expected to be the first parser function called to read tokens from the input file. The function assumes that the first token line is the dimensions of the soup of letters and then reads the subsequent lines to form the soup of letters.
    */
    ...
    */
    vector<string> consume_file(ifstream& input_file) {
        ...
        // The last token line is always an empty line. To see this read the definition of a token line in is_token_line function. Consequently, when an empty file is read, it returns an empty string which has to be removed.
    ...
    /*
        It is assumed that before this function is called, the soup block of tokens was already read from the input file.
        
        Basically a wrapper function of consume_file() to convey a clearer meaning of what consume_file() is used for.
